extends SceneTree
const Art := preload("res://scripts/units/poorper_directional_art.gd")
...
func check(ok: bool, label: String) -> void:
	if not ok:
		failures += 1
		push_error("[Poorper] " + label)

func run() -> void:
	var hashes: Array = []
	for direction in Art.DIRECTIONS:
		var image := Image.load_from_file(Art.ROOT + "poorper_" + direction + ".png")
		...
		print("[Poorper] asset PASS ", direction)
	...
	check(unit.unit_archetype == &"poorper" and unit.move_speed == 145, "Original gameplay identity")
	for i in 8:
	...
	unit.queue_free()
	await process_frame
	print("[Poorper] failures=", failures)
