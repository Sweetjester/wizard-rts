extends SceneTree
const Puppet := preload("res://tools/steel_force/steel_knight_puppet.gd")
...
func run() -> void:
	...
	for direction in 8:
		...
		assert(sheet.save_png(Puppet.ROOT + "steel_knight_" + Puppet.DIRECTIONS[direction] + ".png") == OK)
		print("[KnightBake] ", Puppet.DIRECTIONS[direction], " 40 frames")
	viewport.queue_free()
	await process_frame
	quit()
