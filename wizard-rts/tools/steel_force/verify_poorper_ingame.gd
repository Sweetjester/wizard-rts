...
func run() -> void:
	...
	var scene: PackedScene = factory._scene_for_unit(&"poorper")
	...
	await capture("poorper_ingame_directions")
	...
	check(carrier.board(actors[1]) and actors[1].is_banished(), "Original Poorper boards transport")
	var passenger_page: Texture2D = actors[1].get_node("ArtSprite").texture
	carrier.activate_unload()
	check(not actors[1].embarked and actors[1].get_node("ArtSprite").texture == passenger_page, "Passenger returns with eight-direction art")
	carrier.queue_free()
	for i in 8:
	...
	view._sync_unit_sprites(actors)
	await capture("poorper_ingame_attacks")
	var slain := actors[4]
	...
	root.content_scale_size = root.size
	await capture("poorper_ingame_720")
	root.remove_meta("observer_archive_open")
	stage.queue_free()
	for i in 5: await process_frame
	print("[PoorperLive] failures=", failures)
