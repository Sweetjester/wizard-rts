...
func load_direction(index: int) -> void:
	...
	# The eye correction also changed a living pose; consume only its corpse.
	corpse_source = load(ROOT + "sources/sw_corpse_fix.png") if index == 3 else source
	...
	for i in 5:
		assert(bounds[i].size.y > 30, "Missing Poorper pose " + str(i))
...
func _draw() -> void:
	...
	for polygon in pieces[pose]:
		...
		draw_polygon(vertices, PackedColorArray([Color.WHITE]), uvs, corpse_source if pose == 4 else source)
