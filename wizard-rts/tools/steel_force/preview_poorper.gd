...
func run() -> void:
	...
	for row in 4:
		for i in 8:
			...
			sprite.texture = load(Art.ROOT + "poorper_" + Art.DIRECTIONS[i] + ".png")
			sprite.hframes = 8
			sprite.vframes = 5
	...
	if not output.is_empty():
		root.get_texture().get_image().save_png(output + "/poorper_8direction_contacts.png")
	for f in 8:
		for i in 8: sprites[8 + i].frame = 8 + f
		await process_frame
		await RenderingServer.frame_post_draw
		if not output.is_empty(): root.get_texture().get_image().save_png(output + "/poorper_walk_%02d.png" % f)
	stage.queue_free()
	await process_frame
	quit()
