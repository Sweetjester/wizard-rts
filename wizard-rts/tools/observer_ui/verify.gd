...
func run() -> void:
	...
	check(archive.grid.get_child_count() == 2, "Tier I must contain only its two Oaven forms")
	...
	check(archive.grid.get_child_count() == 3, "Tier II hand must contain three forms")
	check(not Records.record_for(&"spawner", "Creations", build, world, session).sealed, "Tier unlock failed")
	await shot("vault_unsealed")
	var lifted: Button = archive.grid.get_child(1)
	lifted.mouse_entered.emit()
	await shot("vault_hand_hover")
	check(lifted.position.y < lifted.home_position.y, "Hover should lift card from hand")
	lifted.pressed.emit()
	check(archive.selected_id == &"winged_mangler", "Card selection must open that unit")
