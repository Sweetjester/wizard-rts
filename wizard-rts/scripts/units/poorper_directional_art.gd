...
func _ready() -> void:
	...
	hframes = 8
	vframes = 5
	scale = Vector2.ONE * .56
	offset = Vector2(0, -92)
	set_meta("foot_anchor_y", 220.0)
	set_meta("billboard_pixel_size", .009333333)
...
func _update_page() -> void:
	...
	if not _pages.has(key): _pages[key] = load(ROOT + "poorper_" + key + ".png")
	texture = _pages[key]
	_page_key = key
	flip_h = false
...
func _process(delta: float) -> void:
	...
	var index := int(clock * (8.0 if unit.moving else 5.0)) % 8
	if unit.attack_visual_age < .7:
		row = 2
		index = clampi(int(unit.attack_visual_age / .7 * 8), 0, 7)
