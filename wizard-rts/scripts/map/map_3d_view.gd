...
func _ready() -> void:
	...
	# The map is generated by MapBootstrap during the same frame this node is
	# added, so terrain is drawn deferred rather than in _ready.
...
func _sync_unit_sprites(units: Array[Node2D]) -> void:
	for i in units.size():
		...
		if sprite.texture != null:
			cell_height = float(sprite.texture.get_height()) / float(sprite.vframes) * SPRITE_PIXEL_SIZE
		transform.origin.y += cell_height * 0.5
