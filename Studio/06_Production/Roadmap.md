...
- **Roguelike run structure** — design target: class selection [done], procedural map [done], randomized objective [first pass done — only 2 objective types exist, §9/§28 list ~9], map-discovered upgrades researched back at the tower (§17-18, still open — the Terrible Vault's "research" is still 4 hardcoded flat stat buffs bought outright, not upgrades discovered on the map, and is a separate system from the new wizard-leveling choices). Real scope of work remains here — this isn't closed, just less empty.
