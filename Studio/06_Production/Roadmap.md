...
- **Roguelike run structure** — design target: class selection [done], procedural map [done], randomized objective [first pass done — only 2 objective types exist, §9/§28 list ~9], map-discovered upgrades [first pass done, see above]. Still open: the Terrible Vault's "research" is still 4 hardcoded flat stat buffs bought outright, a separate system from wizard leveling/relics and not itself tied to anything found on the map.
