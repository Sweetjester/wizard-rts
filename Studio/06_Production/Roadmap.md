...
- **Art Bible landed 2026-08-11** (`wizard-rts/DARK_FOREST_FRONTIER_V2_ART_BIBLE.md`) — locks a two-tier palette (dark-ink low ground / glass-mushroom high ground) every future prompt must follow. Three concrete follow-ups it identifies, none done yet: (1) regenerate `ancient_tree_hero_b` and `twisted_root_blocker_b` — both fail the bible's silhouette rule and are two of the three most-placed categories on the live map; (2) build the actual high-ground tier — `HIGH_GROUND_TILE` is currently just a tinted low-ground texture, no prop has been generated against the glass-mushroom palette; (3) a real full-coverage pass — most of the map is still pre-pipeline procedural assets, only ~10 of 30+ categories have been touched. Also still wanted: environmental animation (wind-sway trees, glow-pulse on emissive materials) — documented as intent in the bible but not implemented, static-only pipeline today.
